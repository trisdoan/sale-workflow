    ...
    def action_accept(self):
        ...
        for wiz_line in self.line_ids:
            ...
            if wiz_line.sale_line_id:
                ...
                sequence += 1
                vals = wiz_line._prepare_create_so_line(sequence)
...
class SaleOrderRecommendationLine(models.TransientModel):
    ...
    def _prepare_update_so_line(self):
        ...
        if not self.product_uom_readonly:
            vals.update({"product_uom": self.sale_uom_id.id})
        return vals

    def _prepare_create_so_line(self, sequence):
        vals = {
        ...
        if self.wizard_id.sale_recommendation_price_origin == "last_sale_price":
            vals.update({"price_unit": self.price_unit})
        return vals
